...
class Arithmetic:
    ...
    def __adi(self,arg:list):
        self.__register['A'] = encode(filter(self.__register['A']) +  filter(arg[0]))
        self.__check_flag('C')
    
    def __dad(self,arg:list):
        self.__memory_address[self.__rp()] = encode(filter(self.__memory_address[self.__rp()]) +  filter(self.__memory_address[self.__rp(arg[0])])) 
    ...
    def __sui(self,arg:list):
        self.__register['A'] = encode(abs(filter(self.__register['A']) -  filter(arg[0])))
    ...
    def __inx(self,arg:list):
        self.__memory_address[self.__rp(rp)] = encode(filter(self.__memory_address[self.__rp(arg[0])]) + 1) 
